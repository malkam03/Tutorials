...
int main( int argc, char *argv[] ){
  ...
  if( rank < rest ) loc_size++;
  else offset = rest;

  mat = (double *) malloc( loc_size * SIZE * sizeof( double ) );
  memset( mat, 0, loc_size * SIZE * sizeof( double ) );

  /* Initilize local matrix */
  for( i = 0; i < loc_size; i++) {

    i_global = i + ( loc_size * rank + offset );

    for (j = 0; j < SIZE; j++) {
      
      if( j == i_global ) mat[ i * SIZE + j ] = 1.0; 
    }
